...
incorrect. Figure~\ref{fig:seam} illustrates this for $k=21$ ($h=10$) and
$P=4$ processes.

\begin{figure}[H]
\centering
\begin{tikzpicture}[font=\scriptsize, xscale=0.7]
  % Full image outline
  \draw[thick] (0,0) rectangle (10,8);
  \node[above] at (5,8.05) {\small\textbf{Image ($H$ rows, $P=4$ chunks)}};

  % Chunk colours
  \fill[tileA!60]   (0,6) rectangle (10,8);
  \fill[tileB!60]   (0,4) rectangle (10,6);
  \fill[tileC!60]   (0,2) rectangle (10,4);
  \fill[tileD!60]   (0,0) rectangle (10,2);

  % Chunk labels
  \foreach \yc/\lbl in {7/Rank 0 (rows 0--H/4),
                         5/Rank 1 (rows H/4--H/2),
                         3/Rank 2 (rows H/2--3H/4),
                         1/Rank 3 (rows 3H/4--H)} {
    \node[right] at (10.15,\yc) {\lbl};
  }

  % Seam zones (affected rows, top h rows of each non-root chunk)
  \foreach \ytop in {6,4,2} {
    \fill[seambg, opacity=0.85]
      (0,\ytop) rectangle (10,\ytop+0.6);
    \draw[warncol, thick, dashed]
      (0,\ytop) -- (10,\ytop);
    \node[warncol, left, font=\tiny] at (0,\ytop+0.3)
      {$h=10$ affected rows};
  }

  % Arrow pointing to seam
  \draw[-{Stealth}, warncol, thick] (-1.5, 6.3) -- (-0.05, 6.3);
  \node[warncol, left, align=right, font=\tiny] at (-1.6, 6.3)
    {boundary rows clamped\\to local row 0\\(missing rank above)};
\end{tikzpicture}
\caption{MPI seam artefact for $P=4$ and $k=21$ ($h=10$). Red-shaded bands
         mark the $h$ top rows of each non-root chunk whose upper-neighbour
         lookups are incorrectly clamped. With $k=3$ ($h=1$), only 1 row
         per seam is affected, making the RMSE negligible.}
\label{fig:seam}
\end{figure}
