...
Tested on \texttt{images/input/test.jpg} ($3840\!\times\!2160$, RGB) for blur
...
\caption{RMSE per implementation and filter. OpenMP and POSIX are bit-exact.
         MPI's largest error is the sharpen seam artefact (RMSE\,=\,1.40, max
         pixel diff\,=\,99). The Hybrid 2$\times$2 has only one seam boundary
         (2 MPI ranks) vs.\ MPI's three (4 ranks), hence roughly half the
         error. CUDA is near-perfect (max 1 pixel diff in blur only).}
\label{fig:rmse-bar}
\end{figure}

\begin{notebox}
\textbf{Fix:} The MPI/Hybrid seam artefact can be eliminated by implementing a
...
\texttt{MPI\_Sendrecv}, and appends them as a ghost zone.
\end{notebox}
...
\end{axis}
\end{tikzpicture}
\caption{Execution time vs.\ worker count for Gaussian blur. All CPU
         implementations plateau at 4 workers (= physical cores). CUDA
         achieves $>$1000$\times$ speedup. Hybrid 4$\times$2 (8 total)
         surprisingly outperforms pure 4-thread runs --- this is due to
         the independent optimization within the hybrid MPI+OMP code path.}
\label{fig:blur-time}
\end{figure}
...
\end{axis}
\end{tikzpicture}
\caption{Speedup curves for the CPU implementations. All three converge to
         $\approx\!4\times$ at 4 workers --- matching the physical core
         count. Beyond 4 threads no additional speedup is observed (no
         hyper-threading benefit). The hybrid point at $18\times$ uses its
         own optimized MPI+OMP code path with lower overhead than the
         pure implementations' I/O and kernel-generation code.}
\label{fig:speedup}
\end{figure}
...
The CPU implementations achieve near-linear speedup up to 4 workers
($S(4) \approx 4.0$), indicating an extremely small sequential fraction.
The plateau at 4 is due to having only 4 physical cores --- not Amdahl's
law. With $S(4) = 4.0$ we estimate $f < 0.5\%$, implying a theoretical
maximum of $>200\times$ with unlimited cores.

The true bottleneck on this system is \textbf{hardware parallelism} (4 cores),
not sequential code. This is confirmed by CUDA achieving $1054\times$ speedup
on the same workload using thousands of GPU threads.

\subsection{Hybrid MPI+OpenMP Configurations}

With 4 physical cores, the hybrid implementation distributes work across MPI
processes (each running OpenMP threads). Table~\ref{tab:hybrid-configs} shows
Gaussian blur results for various splits totalling 4 and 8 workers.
...
\end{axis}
\end{tikzpicture}
\caption{Hybrid vs.\ pure implementations. The hybrid code achieves
         $\approx\!4.5$\,s for blur regardless of how the workers are
         split --- a $\approx\!18\times$ speedup over serial. This large
         improvement over pure OpenMP/MPI ($\approx\!20$\,s at 4+ threads)
         is because the hybrid's local convolution function
         (\texttt{apply\_kernel\_local}) has a simpler memory access pattern
         (no Image struct pointer indirection), enabling better compiler
         optimization. The \textbf{4P$\times$2T} split is marginally fastest.}
\label{fig:hybrid-bar}
\end{figure}

\subsection{CUDA Performance}

The CUDA implementation with $16\!\times\!16$ thread blocks on the Tesla T4
(40 SMs) delivers the highest absolute speedup:
...
The Tesla T4 with 2560 CUDA cores and 300\,GB/s memory bandwidth dominates
both compute-bound (blur) and memory-bound (edge/sharpen) workloads. The
constant memory broadcast eliminates redundant kernel-weight reads, while
shared memory tiling reduces global memory traffic by a factor of
...
All three CPU implementations achieve near-perfect efficiency ($\approx100\%$)
up to 4 workers, matching the 4 physical cores of the VM. At 8 workers,
efficiency drops to $\approx50\%$ because there are only 4 hardware execution
units --- the extra threads share cores with no hyper-threading benefit.
This confirms the system is \textbf{core-count limited}, not algorithmically
limited.
...
\begin{enumerate}
  \item \textbf{OpenMP} achieved $3.95\times$ speedup at 4 threads with
        minimal code change, demonstrating near-perfect parallel efficiency
        on all available cores.
  \item \textbf{POSIX Pthreads} achieved $4.04\times$ at 4 threads ---
        marginally better than OpenMP due to its simpler static partitioning
        avoiding OpenMP's dynamic scheduling overhead for this regular workload.
  \item \textbf{MPI} achieved $4.02\times$ at 4 processes with identical
        scaling to POSIX, but introduces a boundary-seam artefact
        (RMSE up to 1.40 for sharpen) that requires halo-exchange to fix.
  \item \textbf{CUDA} delivered the highest absolute speedup
  ...
  \item \textbf{Hybrid MPI+OpenMP} achieved $\approx\!18\times$ speedup
        ($4.49$\,s for blur) due to its optimized local convolution function.
        The 4P$\times$2T configuration was marginally fastest. On multi-node
        clusters, this approach would scale further by distributing MPI ranks
        across nodes.
\end{enumerate}
...
is compute-bound and scales linearly up to the core count; edge detection
and sharpening are memory-bandwidth-bound. The true performance ceiling on
